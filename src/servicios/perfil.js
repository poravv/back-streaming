...
routes.get('/getsql/', async (req, res) => {
    await database.query('select * from perfil order by descripcion asc', { type: QueryTypes.SELECT })
        .then((resultado) => {
            res.json({
...
routes.get('/get/', async (req, res) => {
    await perfil.findAll({where:{ state: 'AC' }}).then((resultado) => {
        res.json({
            mensaje: "successfully",
            body: resultado
        })
...
routes.get('/getall/', async (req, res) => {
    await perfil.findAll().then((resultado) => {
        res.json({
            mensaje: "successfully",
            body: resultado
        })
...
routes.get('/getone/', async (req, res) => {
    await perfil.findOne({where:{ state: 'AC' }}).then((resultado) => {
        res.json({
            mensaje: "successfully",
            body: resultado
        })
...
routes.get('/get/:idperfil', verificaToken, async (req, res) => {
    await perfil.findByPk(req.params.idperfil).then((resultado) => {
        res.json({
            mensaje: "successfully",
            body: resultado
        })
...
routes.put('/put/:idperfil', verificaToken, async (req, res) => {

    const t = await database.transaction();
    try {
        await perfil.update(req.body, { where: { idperfil: req.params.idperfil } }, {
            transaction: t
        }).then((resultado) => {
            jwt.verify(req.token, process.env.CLAVESECRETA, (errorAuth, authData) => {
                if (!validateNivel({ authData: authData })) {
...
routes.delete('/del/:idperfil', verificaToken, async (req, res) => {
    const t = await database.transaction();
    try {
        await perfil.destroy({ where: { idperfil: req.params.idperfil } }, {
            transaction: t
        }).then((resultado) => {
            jwt.verify(req.token, process.env.CLAVESECRETA, (errorAuth, authData) => {
                if (!validateNivel({ authData: authData })) {
